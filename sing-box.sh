...
# =========================
# 获取最新版本（stdout 只输出版本号；日志到 stderr）
# =========================
get_latest_version() {
  echo "🔍 正在检查最新版本..." >&2
  VERSION_TAG=$(
  ...
  if [ -z "$VERSION_TAG" ] || [ "$VERSION_TAG" = "null" ]; then
    ...
    echo "💡 可能原因：网络问题、GitHub API 限流、或防火墙拦截" >&2
    return 1
  fi
...
get_current_version() {
  ...
  version=$("$INSTALL_DIR/sing-box" version 2>/dev/null | sed -n 's/.*version[[:space:]]\([0-9.]\+\).*/\1/p' | head -n1)
...
install_alpine() {
  ...
  echo "🔐 正在生成密钥..."
  KEYS=$("$INSTALL_DIR/sing-box" generate reality-keypair 2>/dev/null || true)
  # 兼容 PrivateKey/PublicKey 和 private_key/public_key
  PRIVATE_KEY=$(echo "$KEYS" | sed -n 's/.*[Pp]rivate[_ ]*[Kk]ey[":]*[[:space:]]*\([A-Za-z0-9+\/=._-]*\).*/\1/p' | head -n1)
  PUBLIC_KEY=$( echo "$KEYS" | sed -n 's/.*[Pp]ublic[_ ]*[Kk]ey[":]*[[:space:]]*\([A-Za-z0-9+\/=._-]*\).*/\1/p'  | head -n1)
  if [ -z "$PRIVATE_KEY" ] || [ -z "$PUBLIC_KEY" ]; then
    echo "❌ 解析 reality-keypair 失败" >&2
    exit 1
  fi
